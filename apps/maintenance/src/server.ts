...
import { createDispatcher } from "./mastra/engine/dispatch.js";
import { startSlackConnector } from "./mastra/connectors/slack/connector.js";
 ...
 *   `pnpm -C apps/maintenance studio`  →  `mastra studio -s 4111`
 */
...
serve({ fetch: app.fetch, port }, (info) => {
  console.log(`[server] Last Light listening on http://localhost:${info.port}`);
  console.log(`[server] Studio: pnpm -C apps/maintenance studio  (→ mastra studio -s ${info.port})`);
});
...
if (slack) {
  const dispatch = createDispatcher(mastra);
  startSlackConnector(slack, dispatch).catch((err: unknown) => {
