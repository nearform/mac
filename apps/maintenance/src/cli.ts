...
/**
 * Last Light CLI — thin client that triggers the running Mastra server.
 *
 * Ported from lastlight `src/cli.ts`, adapted for the Mastra port:
 ...
 *
 * Usage:
 *   tsx src/cli.ts <github-url>            Triage that issue (default — cheap, M6)
 *   tsx src/cli.ts <owner/repo#number>     Same, shorthand
 *   tsx src/cli.ts build <github-url>      Run the FULL build cycle (architect→PR)
 *   tsx src/cli.ts build <owner/repo#N>    Same, shorthand
 *   tsx src/cli.ts review <owner/repo#N>   Review that PR (pr-review workflow)
 *
 * The CLI does NOT run agents — it POSTs to the server. Start it first:
 *   pnpm -C apps/maintenance dev
 */

const SERVER_URL = process.env.LASTLIGHT_URL || "http://localhost:4111";
...
Usage:
  tsx src/cli.ts <github-url>          Triage that one issue (default — cheap, M6)
  tsx src/cli.ts <owner/repo#number>   Same, shorthand
  tsx src/cli.ts build <github-url>    Run the FULL build cycle (architect/executor/reviewer/PR)
  tsx src/cli.ts build <owner/repo#N>  Same, shorthand
  tsx src/cli.ts review <owner/repo#N> Review that PR

The server must be running (pnpm -C apps/maintenance dev). Set LASTLIGHT_URL to override.
`);
  process.exit(0);
}
...
async function main() {
  ...
  try {
    ...
    console.error(`Start it first: pnpm -C apps/maintenance dev`);
    process.exit(1);
  }
  ...
  if (firstArg === "review") {
    ...
    const parsed = target ? parseGitHubRef(target) : null;
    if (!parsed) {
      console.error(`Usage: tsx src/cli.ts review <owner/repo#N> | <pr-url>`);
      process.exit(1);
    }
    const { owner, repo, number } = parsed;
  ...
  if (!parsed) {
    ...
    console.error(`For a full build cycle: tsx src/cli.ts build ${firstArg}`);
    process.exit(1);
  }
  const { owner, repo, number, type } = parsed;
  const isPr = type === "pr";
  console.log(`Triggering ${isPr ? "PR review" : "issue triage"} for ${owner}/${repo}#${number}...`);
  if (!isPr) console.log(`(For a full build cycle: tsx src/cli.ts build ${owner}/${repo}#${number})`);
  await post("/cli/run", {
