...
/**
 * Slack connector (Socket Mode) — the Mastra-native re-home of lastlight's
...
/** A function that consumes a normalized event (the dispatcher). */
...
export class SlackConnector {
  ...
  async start(): Promise<void> {
    ...
    // the initiating thread (engine/notify.ts → build/pr-review feedback).
 ...
 * or null when Slack isn't configured. Called once at boot from server.ts.
 */
