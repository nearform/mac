...
import { approvalLink } from "../config.js";
import { postStatus, updateStatus, postMessage } from "../connectors/slack/notify.js";
import { parseGuardrails } from "../agents/guardrails.js";
...
/**
 * build workflow — ported from lastlight `workflows/build.yaml`:
 * guardrails → architect → (post_architect approval gate) → executor →
 * reviewer fix-loop → finalize → PR.
 *
 * GITHUB-CENTRIC
 * --------------
 * The build treats the source issue + branch as the primary surface (like
 * lastlight):
 *  - Each phase COMMITS its work (code + `.lastlight/<issueDir>/` artifacts) and
 *    PUSHES the `lastlight/issue-<n>` branch, so progress is visible on GitHub
...
const triggerSchema = z.object({
  ...
  // Bootstrap bypass (lastlight parity): when the TASK is to set up the missing
...
const buildState = z.object({
  ...
  // ../config approvalLink + ../server/approval.ts).
  runId: z.string(),
  branch: z.string(),
  issueDir: z.string(),
  ready: z.boolean(),
...
function artifactsTreeUrl(st: BuildState): string {
  return `https://github.com/${st.owner}/${st.repo}/tree/${st.branch}/.lastlight/${st.issueDir}`;
}
...
function artifactUrl(st: BuildState, name: string): string {
  return `https://github.com/${st.owner}/${st.repo}/blob/${st.branch}/.lastlight/${st.issueDir}/${name}`;
}
...
function renderStatusComment(st: BuildState): string {
  const lines = st.progress.map((e) => {
    ...
    if (e.key === "approval" && e.status === "awaiting") {
      line +=
        ` — [✅ Approve](${approvalLink(st.runId, "approve")})` +
        ` · [❌ Reject](${approvalLink(st.runId, "reject")})`;
    }
    return line;
  });
  return [
    `### 🤖 Last Light — build for #${st.issueNumber}`,
    "",
    ...
    ...lines,
    "",
    `Artifacts: [.lastlight/${st.issueDir}/](${artifactsTreeUrl(st)}) — updates as the build runs.`,
  ]
    .filter((l) => l !== "")
    .join("\n");
}
...
function requireAppConfig(): GitHubAppConfig {
...
function mintReadToken(): Promise<string> {
  return mintTokenForProfile(requireAppConfig(), "read").then((t) => t.token);
}

/** WRITE token (`build` → repo-write: contents + pull_requests) for push + PR. */
function mintWriteToken(): Promise<string> {
  return mintTokenForProfile(requireAppConfig(), resolveProfile("build")).then((t) => t.token);
}
...
/** Artifact path under the per-issue `.lastlight/` subfolder. */
...
async function publishPhase(
  ...
  if (st.slackChannel && st.slackThread) {
    if (st.slackStatusTs) {
      await updateStatus(st.slackChannel, st.slackStatusTs, body);
    } else {
      const ts = await postStatus({ channel: st.slackChannel, thread: st.slackThread }, body);
  ...
  try {
    ...
    if (opts.reaction) {
      try {
        await addIssueReaction(octokit, {
          owner: st.owner,
          repo: st.repo,
    ...
    try {
      if (st.statusCommentId) {
        ...
        const c = await addIssueComment(octokit, {
          owner: st.owner,
          repo: st.repo,
          number: st.issueNumber,
          body,
...
async function slackTerminal(st: BuildState): Promise<void> {
  if (!st.slackChannel || !st.slackThread) return;
...
const guardrailsStep = createStep({
  ...
  execute: async ({ inputData, runId, tracingContext, mastra }) => {
    ...
    // FRESH branch off base with no prior run's `.lastlight/` state on it
    ...
    const branch = `lastlight/issue-${inputData.issueNumber}-${shortRun}`;
    ...
    const deps = await installDependencies(ws);
    console.log(
      `[build] deps install: ran=${deps.ran} ok=${deps.ok} pm=${deps.packageManager}`,
    );
    await writeArtifact(ws, artifact(state, "deps-install.log"), deps.output);

    // Pre-flight: confirm the checkout has a usable test command.
    const agent = mastra.getAgent("guardrails");
    ...
    const upd = await publishPhase(result, ws, {
      ...
      commitMessage: `chore(lastlight): guardrails for #${result.issueNumber}`,
    });
    return { ...result, ...upd };
  },
});
...
const architectStep = createStep({
  ...
  execute: async ({ inputData, tracingContext, mastra }) => {
    ...
    const ws = taskWorkspace(st.taskId);
    await ws.init();
    ...
    const upd = await publishPhase(result, ws, {
      ...
      commitMessage: `docs(lastlight): architect plan for #${st.issueNumber}`,
    });
    return { ...result, ...upd };
  },
});
...
const executorStep = createStep({
  ...
  execute: async ({ inputData, tracingContext, mastra }) => {
    ...
    const agent = mastra.getAgent("executor");
    const res = await agent.generate(
      [
    ...
    const upd = await publishPhase(result, ws, {
      ...
      status: "done",
      detail: "implemented",
      commitMessage: `feat(lastlight): implement #${st.issueNumber}`,
    });
    return { ...result, ...upd };
  },
});
...
const fixStep = createStep({
  ...
  execute: async ({ inputData, tracingContext, mastra }) => {
    ...
    st = {
      ...st,
      ...(await publishPhase(st, ws, {
        commitMessage: `fix(lastlight): apply review cycle ${cycle} for #${st.issueNumber}`,
      })),
    };
    return st;
  },
...
const reviewStep = createStep({
  ...
  execute: async ({ inputData, tracingContext, mastra }) => {
    ...
    const reviewer = mastra.getAgent("build-reviewer");
    ...
    result = {
      ...
      ...(await publishPhase(result, ws, {
        commitMessage: `chore(lastlight): review cycle ${reviewNum} for #${st.issueNumber}`,
      })),
    };
    return result;
  },
  ...
  execute: async ({ inputData, tracingContext, mastra }) => {
    ...
    if (st.aborted) {
      ...
      const upd = await publishPhase(st, ws, {
        commitMessage: `chore(lastlight): aborted #${st.issueNumber}`,
      });
      return { ...st, ...upd };
    }
    ...
    const upd = await publishPhase(result, ws, {
      commitMessage: `chore(lastlight): build result for #${st.issueNumber}`,
    });
    return { ...result, ...upd };
  },
});
  ...
  execute: async ({ inputData, tracingContext, mastra }) => {
    ...
    const body = [
      ...
      st.lastVerdict === "REQUEST_CHANGES"
        ? `\n> ⚠️ Unresolved reviewer issues remain — see \`.lastlight/${st.issueDir}/reviewer-verdict.md\`.`
        : "",
      "",
      `## Planning & execution artifacts (on this branch, \`.lastlight/${st.issueDir}/\`)`,
    ...
    const upd = await publishPhase(result, ws, {
      ...
      commitMessage: `chore(lastlight): PR for #${st.issueNumber}`,
...
export const buildWorkflow = createWorkflow({
  id: "build",
  inputSchema: triggerSchema,
  outputSchema: buildState,
})
