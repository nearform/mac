...
/**
 * Programmatic trigger endpoints for the CLI (src/cli.ts) — the Mastra re-home
 * of lastlight's `/api/build` + `/api/run` routes (which were mounted on the
...
// NOTE: no custom /health route — Mastra's built-in server already serves
// `GET /health` ({ success: true }) at the root. The CLI pings that.
