...
/**
 * GitHub App token broker — ported from lastlight `src/engine/git-auth.ts`,
 * trimmed to the token-minting core. We deliberately DROP the global
 * `~/.gitconfig` credential-helper writing (lastlight's opt-in
 * `LASTLIGHT_WRITE_GLOBAL_GIT` path): in the Mastra spike the coding agent runs
...
/** Read GitHub App config from the environment (lastlight-compatible names). */
