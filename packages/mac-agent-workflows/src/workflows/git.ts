 ...
 * work and pushes a per-RUN branch `mac/issue-<n>-<runId>` — a fresh ref
 * each build, so it starts from clean base with no prior run's `.mac/`
 ...
 * ancestor git repo (the monorepo itself is a git repo), and `--is-inside-work-tree`
...
export async function cloneRepo(
  ...
  await runInWorkspace(ws, "git", ["config", "user.email", "mac-nf@users.noreply.github.com"]);
  await runInWorkspace(ws, "git", ["config", "user.name", "MAC"]);
}

/**
 * Create the working branch fresh off the cloned base HEAD.
 *
 * The branch is per-RUN (`mac/issue-<n>-<runId>` — see build.ts), so it's
 * always a NEW ref: every build starts from a clean base with no prior run's
 * `.mac/` state (guardrails report, plan, etc.) on it. That clean start is
...
/**
 * Pathspec for code diffs: exclude our own `.mac/` artifacts AND dependency
 ...
 * workflow's own `.mac/` artifacts so the reviewer sees only real code.
 */
...
/** Names of code files changed vs the base branch (excludes `.mac/`). */
...
/**
 * Write a build artifact into the checkout's `.mac/` dir — the workflow's
 * the per-phase markdown (guardrails-report.md,
...
export async function writeArtifact(
  ...
  await fs.writeFile(`.mac/${relPath}`, content, {
    recursive: true,
    overwrite: true,
  });
}
...
/**
 * Live-PR mode only: stage everything (code + `.mac/` artifacts) and
...
/**
 * Per-phase publish: stage + commit everything (code + `.mac/` artifacts)
